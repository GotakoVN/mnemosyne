...
if __name__ == "__main__":
    ...
    print("\n  GROUP 1: Normal Operation (10 tests)")
    print("  " + "─" * 40)
    test("Full dashboard output", test_full_dashboard)
    test("Compact mode", test_compact_mode)
    test("JSON mode", test_json_mode)
    test("Save snapshot", test_save_snapshot)
    test("Trends display", test_trends)
    test("Auto-snapshot on full run", test_auto_snapshot)
    test("Health score format", test_health_score_in_output)
    test("DB size in output", test_db_size_in_output)
    test("Quality indicators section", test_quality_indicators_section)
    test("Recommendations section", test_recommendations_section)

    print("\n  GROUP 2: Edge Cases (10 tests)")
    print("  " + "─" * 40)
    test("Invalid flag handling", test_invalid_flag)
    test("Multiple flags", test_multiple_flags)
    test("Missing DB path", test_empty_db_path)
    test("Corrupted snapshot file", test_corrupted_json_snapshot)
    test("Empty snapshot dir", test_empty_snapshot_dir)
    test("Special characters in memories", test_special_characters_in_memory)
    test("Output size reasonable", test_large_output)
    test("JSON structure valid", test_json_valid_structure)
    test("Snapshot JSON valid", test_snapshot_json_valid)
    test("Concurrent access safety", test_concurrent_access)

    print("\n  GROUP 3: Integration (5 tests)")
    print("  " + "─" * 40)
    test("Wiki count matches files", test_wiki_count_matches)
    test("WM count matches DB", test_db_count_matches)
    test("Episodic count matches DB", test_episodic_count_matches)
    test("Triples count matches DB", test_triples_count_matches)
    test("Consolidation count matches DB", test_consolidation_count_matches)

    print("\n  GROUP 4: Stress / Boundary (5 tests)")
    print("  " + "─" * 40)
    test("Rapid fire (10 runs)", test_rapid_fire)
    test("JSON pipe to python", test_json_pipe_to_python)
    test("Output encoding (UTF-8)", test_output_encoding)
    test("Performance (<5s)", test_performance)
    test("Snapshot growth tracking", test_snapshot_growth)

    print("\n  GROUP 5: Data Integrity (5 tests)")
    print("  " + "─" * 40)
    test("Importance dist sums to total", test_importance_distribution_sums)
    test("Recall dist sums to total", test_recall_distribution_sums)
    test("Noise % calculation correct", test_noise_pct_calculation)
    test("Global count accuracy", test_global_count_accuracy)
    test("Quality score in bounds", test_quality_score_bounds)
